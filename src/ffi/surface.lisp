...
(defsdl2-lispfun sdl-must-lock (surface)
  (if (zerop (logand (cffi:foreign-slot-value surface 'sdl-surface 'flags)
                     +sdl-rleaccel+))
      0
      1))

(defsdl2type sdl-blit :pointer
...
(defsdl2fun ("SDL_DuplicateSurface" sdl-duplicate-surface) (:pointer sdl-surface)
  (surface (:pointer sdl-surface)))
...
(defsdl2fun ("SDL_SoftStretch" sdl-soft-stretch) :int
  (src (:pointer sdl-surface))
  (src-rect (:pointer sdl-rect))
  (dst (:pointer sdl-surface))
  (dst-rect (:pointer sdl-rect)))
